/**
 * MueblePro Enterprise — Board Optimization Engine
 * Guillotine Bin Packing for standard boards.
 */
...
/**
 * Run board optimization on a cut list.
 * @param {Array} cutList - Items from the design engine
 * @param {number} boardWidth - Board width (default 2440mm)
 * @param {number} boardHeight - Board height (default 1830mm)
 * @param {number} kerf - Saw blade width (default 3mm)
 * @param {boolean} allowRotation - Allow part rotation
    ...
    while (partsToNest.length > 0 && safetyLoop < MAX_BOARDS) {
        ...
        for (const part of partsToNest) {
            ...
            if (!fitsNormal && !fitsRotated) {
                unplacedParts.push({ ...part, reason: `Pieza ${pw}×${ph}mm excede tablero ${boardWidth}×${boardHeight}mm` });
                continue;
            }
        ...
        if (packed.length > 0) {
            ...
            unplacedParts.push(...unpackable.map(u => ({
                ...u, reason: 'No cabe en tablero vacío'
            })));
            break;
        }
    }
